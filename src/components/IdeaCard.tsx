...
export default function IdeaCard({ idea, index = 0 }: { idea: Idea; index?: number }) {
  ...
  return (
    <Link href={`/idea/${idea.id}`} className="no-underline block card overflow-hidden animate-in group cursor-pointer" style={{ animationDelay: `${index * 0.05}s` }}>
      {/* 配图区 */}
      <div className={`card-banner h-36 bg-gradient-to-br ${colorClass} flex items-center justify-center relative overflow-hidden`}>
      ...
      <div className="card-content p-4">
        ...
        <div className="card-tags flex flex-wrap gap-1.5 mb-3">
          {idea.tags.map(tag => (
