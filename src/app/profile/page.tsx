    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="card p-6 mb-6 animate-in">
          <div className="flex items-center gap-4">
            ...
            <div><h1 className="text-xl font-bold text-gray-800">{currentUser?.nickname}</h1><p className="text-sm text-gray-400 mt-0.5 break-words">{currentUser?.bio || '一个热爱脑洞的人'}</p></div>
              ...
              <div className="card p-3 sticky top-4">
                ...
                {showCreateIdeaFolder && (
                  <div className="flex gap-2 mb-2 px-2">
                    <input type="text" placeholder="分组名称" value={newIdeaFolderName} onChange={e => setNewIdeaFolderName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateIdeaFolder() }} className="flex-1 px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-400/30" autoFocus />
                    <button onClick={handleCreateIdeaFolder} className="btn-primary px-2.5 py-1.5 text-xs">创建</button>
                  </div>
                )}
                  ...
                  {ideaFolders.map(folder => (
                    <div key={folder.id} className={`group rounded-xl transition-all ${ideaFolderId === folder.id ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
                      {ideaRenameFolder === folder.id ? (
                        ...
                        <button onClick={() => { setIdeaFolderId(ideaFolderId === folder.id ? null : folder.id); setIdeaCardMenu(null) }} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-left">
              ...
              {filteredIdeaDirIdeas.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-gray-200 animate-in">
                  ...
                  <p className="text-gray-500 font-medium">
                    {ideaFolderId === 'idea_default' ? '还没有发布过脑洞' : '这个分组还是空的'}
                  </p>
                  <p className="text-sm text-gray-400 mt-1">
                    {ideaFolderId === 'idea_default' ? '去右上角发布你的第一个脑洞吧' : '把脑洞移到这里来管理'}
                  </p>
                </div>
              ) : (
      ...
      {showDeleteIdeaFolder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setShowDeleteIdeaFolder(null)}>
          <div className="absolute inset-0 bg-black/30" /><div className="relative bg-white rounded-2xl shadow-xl p-5 w-72 animate-in" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-4"><div className="text-3xl mb-2">🗑️</div><h3 className="text-base font-bold text-gray-800">删除分组</h3><p className="text-sm text-gray-500 mt-1">脑洞将会移回「默认创作」</p></div>
            ...
            <div className="p-2 max-h-52 overflow-y-auto">
              {collectionFolders.filter(f => f.id !== selectedFolderId).map(folder => (
                <button key={folder.id} onClick={() => { selectedIdeaIds.forEach(id => { if (collectionFolders.find(f => f.id === selectedFolderId)?.ideaIds.includes(id)) removeIdeaFromFolder(selectedFolderId, id); addIdeaToFolder(folder.id, id) }); setShowBatchMove(false); exitBatchMode() }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-all text-left">
                  <span className="text-base">{folder.icon}</span><div><p className="text-sm font-medium text-gray-700">{folder.name}</p><p className="text-[11px] text-gray-400">{folder.ideaIds.length} 个脑洞</p></div>
                </button>
              ))}
            </div>
          </div>
